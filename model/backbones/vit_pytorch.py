...
class RandomMix(object):
    ...
    def __call__(self, img, ori_img, mmix, num, label, label_new):
        ...
        if random.uniform(0, 1) > self.probability:
            ...
            mmix = mmix.unsqueeze(2)
            h_shift = random.sample(range(-1,2), 1)
            img[num,:,:] = ori_img[num,:,:] * mmix[select[0,0],:,:] + ori_img[select[0,0],:,:] * (1.0 - mmix[select[0,0],:,:])
...
class TransReID(nn.Module):
    ...
    def forward_features(self, x, mmix, label, label_new, camera_id, view_id):
        B = x.shape[0]
        x = self.patch_embed(x)
        if self.training:
            ori_x = x.clone()
            for i in range (B):
                x, label_new = self.random_mix(x, ori_x, mmix, i, label, label_new)
        ...
        return x[:, 0], label_new

    def forward(self, x, mmix, label, label_new, cam_label=None, view_label=None):
        x, label_new = self.forward_features(x, mmix, label, label_new, cam_label, view_label)
        return x, label_new
...
class TransReID_Mask(nn.Module):
    """ Transformer-based Object Re-Identification
    """
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0., camera=0, view=0,
        ...
        self.norm = norm_layer(embed_dim)
        self.mixfc = nn.Conv2d(768 * 4, 1, kernel_size=1, stride=1, padding=0)
    ...
    def forward_features(self, x, mmix, add_mask, camera_id, view_id):
        B = x.shape[0]
        x = self.patch_embed(x)
        if add_mask == 1:
            x = x * mmix.unsqueeze(2) 
        ...
        for blk in self.blocks:
            ...
            flag += 1
        if add_mask == 0:
            x_final = x[:, 1:].transpose(1, 2).reshape(B, -1, H, W)
            mmix = torch.cat((x1, x3, x9, x_final), 1)
            mmix = self.mixfc(mmix)
            mmix = self.sigmoid(mmix.reshape(B, -1))
        x = self.norm(x)

        return x[:, 0], mmix

    def forward(self, x, mmix, add_mask, cam_label=None, view_label=None):
        x, mmix = self.forward_features(x, mmix, add_mask, cam_label, view_label)
        return x, mmix
...
def vit_mask_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, camera=0, view=0,local_feature=False,sie_xishu=1.5, **kwargs):
    model = TransReID_Mask(
        img_size=img_size, patch_size=16, stride_size=stride_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,\
        camera=camera, view=view, drop_path_rate=drop_path_rate, drop_rate=drop_rate, attn_drop_rate=attn_drop_rate,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),  sie_xishu=sie_xishu, local_feature=local_feature, **kwargs)

...
def do_train(cfg,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (img, vid, target_cam, target_view) in enumerate(train_loader):
            ...
            with torch.no_grad():
                add_mask = 0
                _, mmix = model_pt(img, img, add_mask, cam_label=target_cam, view_label=target_view)
                #print(mmix)
                '''mmix_one = torch.ones(mmix.shape).to(device)
                mmix_zero = torch.zeros(mmix.shape).to(device)
                mmix = torch.where(mmix>=0.5, mmix_one, mmix_zero)
                print(torch.sum(mmix, 1)/(mmix.shape[1]))'''
            with amp.autocast(enabled=True):
                score, feat, target_new = model(img, mmix, target, target_new, cam_label=target_cam, view_label=target_view )
                ...
                loss = loss_fn(score, feat, mmix, sub_hot, sub_tri, target, target_cam)
